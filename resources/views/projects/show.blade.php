...
{{-- END COLLAPSIBLE PROJECT HEADER --}}

{{-- ============================================================
     TASK RISK CALCULATION FUNCTIONS
     All risk colors relative to percent_complete
     ============================================================ --}}
@php
...
<div class="widget-card" id="taskListCard">
    ...
    @foreach(['tab-overdue' => 'Overdue Tasks', 'tab-team' => 'Team', 'tab-resources' => 'Resources', 'tab-files' => 'Files', 'tab-forums' => 'Forums', 'tab-gantt' => 'Gantt Chart', 'tab-log' => 'Workflow Log'] as $tabId => $tabLabel)
...
@section('slideout-panels')
    ...
    <div id="team-slideout" class="slideout-panel">
        ...
        <div class="slideout-content">
            <p class="text-gray-500 text-sm mb-4">{{ $project->name }}</p>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">
                                    @if($member->role) {{ $member->role->role_name }} @endif
                                    @if($member->allocation_percent > 0) · {{ $member->allocation_percent }}% @endif
                                </p>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <select name="owner_id" required
                            ...
                            <option value="{{ auth()->id() }}">
                                {{ auth()->user()->first_name }} {{ auth()->user()->last_name }} (Me)
                            </option>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.confirmDeleteTask = function (taskId) {
        ...
        fetch('{{ route("tasks.destroy", ":id") }}'.replace(':id', taskId), {
            method: 'DELETE',
            headers: {
