        ...
        <div class="flex-1 overflow-y-auto min-h-0">
            <form id="createTicketForm">
                ...
                <div class="px-5 pt-3 pb-2 space-y-2">
                        ...
                        <div>
                            ...
                            <select name="priority" class="ct-input" required>
                                @foreach($priorities as $p)
                                    <option value="{{ $p->level }}" {{ $p->level == 3 ? 'selected' : '' }}>{{ $p->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="status_id" class="ct-input" required>
                                @foreach($statuses as $s)
                                    <option value="{{ $s->id }}" {{ $s->is_default_open ? 'selected' : '' }}>{{ $s->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-4 gap-2">
                            ...
                            <select name="department_id" id="createDeptSelect" class="ct-input">
                                ...
                                @foreach($departments as $dept)
                                    <option value="{{ $dept->id }}"
                                        data-lat="{{ $dept->lat ?? '' }}"
                                        data-lng="{{ $dept->lng ?? '' }}"
                                        data-address="{{ trim(($dept->address_line1 ?? '').' '.($dept->city ?? '').' '.($dept->state ?? '')) }}">
                                        {{ $dept->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="email" name="reporter_email" id="createReporterEmail"
                                   class="ct-input" required value="{{ auth()->user()->email }}">
                        </div>
                        <div>
                    ...
                    {{-- Row 3: Subject (3 cols) · Location (1 col) --}}
                    <div class="grid grid-cols-4 gap-2">
