        ...
        <div class="px-6 py-3 border-b border-gray-100 bg-gray-50 flex items-center gap-3 flex-shrink-0 flex-wrap">
            <div>
                ...
                <select id="timesheetAgent" class="text-sm border border-gray-300 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                    @foreach($agents as $a)
                        <option value="{{ $a->id }}">{{ trim(($a->first_name ?? '') . ' ' . ($a->last_name ?? '')) ?: $a->email }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="text-xs text-gray-500 block mb-0.5">From</label>
                <input type="date" id="timesheetFrom" class="text-sm border border-gray-300 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ now()->startOfMonth()->toDateString() }}">
            </div>
            <div>
                <label class="text-xs text-gray-500 block mb-0.5">To</label>
                <input type="date" id="timesheetTo" class="text-sm border border-gray-300 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ now()->toDateString() }}">
            </div>
            <div class="mt-4">
