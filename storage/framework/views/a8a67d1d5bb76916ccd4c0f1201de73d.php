...
<button id="filtersToggleBtn" onclick="toggleFilterPanel()"
        class="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 text-xs border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors <?php echo e(request()->hasAny(['type','status','priority','assignee','dept']) ? 'bg-blue-50 border-blue-300 text-blue-700' : ''); ?>">
    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/>
    </svg>
    Filters
    <?php if(request()->hasAny(['type','status','priority','assignee','dept'])): ?>
...
<div class="flex items-center gap-1.5 flex-wrap min-w-0">
    <?php if(request('type')): ?>
        <span class="flex items-center gap-1 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full">
            ...
            <a href="<?php echo e(request()->fullUrlWithoutQuery(['type'])); ?>" class="hover:text-blue-900 leading-none">×</a>
        </span>
    <?php endif; ?>
    <?php if(request('priority')): ?>
        <span class="flex items-center gap-1 px-2 py-0.5 bg-orange-100 text-orange-700 text-xs rounded-full">
            ...
            <a href="<?php echo e(request()->fullUrlWithoutQuery(['priority'])); ?>" class="hover:text-orange-900 leading-none">×</a>
        </span>
    <?php endif; ?>
    <?php if(request('status')): ?>
        <?php $st = $statuses->find(request('status')); ?>
        <span class="flex items-center gap-1 px-2 py-0.5 text-xs rounded-full"
              style="background:<?php echo e($st?->color); ?>22;color:<?php echo e($st?->color); ?>">
            <?php echo e($st?->name); ?>

            <a href="<?php echo e(request()->fullUrlWithoutQuery(['status'])); ?>" class="opacity-70 hover:opacity-100 leading-none">×</a>
        </span>
    <?php endif; ?>
    <?php if(request()->has('dept')): ?>
        <?php $deptChip = isset($departments) ? $departments->find(request('dept')) : null; ?>
        <?php if($deptChip): ?>
        <span class="flex items-center gap-1 px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded-full">
            <?php echo e($deptChip->name); ?>

            <a href="<?php echo e(request()->fullUrlWithoutQuery(['dept'])); ?>" class="hover:text-purple-900 leading-none">×</a>
        </span>
        <?php endif; ?>
    <?php endif; ?>
